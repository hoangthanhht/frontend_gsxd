    ...
    >
      <div contenteditable="true" class="baocao">
        <div class="kindBC flex items-center h-24">
          <p class="text-3xl pl-60">BÁO CÁO NGÀY TƯ VẤN GIÁM SÁT</p>
        ...
        <p class="dateBC">
          Ngày {{ getdate[0] }} tháng {{ getdate[1] }} năm {{ getdate[2] }}
        </p>
        <div class="project">
      ...
      this["storeqlda/getListReport"](data).then((rs) => {
        ...
        for (var i in res) {
          if (res.length > 0) {
            ...
            this.tenNT.push(
              Object.keys(this.dataGiamSat[i][this.mucArr[i][1]])
            );
...
.kindBC:hover > .action {
  display: flex;
} /* hover tác động lên phần tử khác */
.dateBC {
  border-bottom: 1px solid;
  text-align: right;
  margin: 0;
